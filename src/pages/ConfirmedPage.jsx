...
function ConfirmedPage() {
    ...
    console.log(orderInfo.user.addresses[0].firstName)
        ...
        orderInfo && (
            ...
            <div className="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full" key={item.id}>
                <div className="pb-4 md:pb-8 w-full md:w-40">
                    <img className="w-full hidden md:block" src={itemArray[0].thumbnail} alt="dress" />
                </div>
                <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
                    <div className="w-full flex flex-col justify-start items-start space-y-8">
                        <h3 className="text-xl xl:text-2xl font-semibold leading-6 text-gray-800">{itemArray[0].title}</h3>
                        <div className="flex justify-start items-start flex-col space-y-2">
                            <p className="text-sm leading-none text-gray-800">
                                <span className="text-gray-300">Brand: </span> {itemArray[0].brand}
                            </p>
                            <p className="text-sm leading-none text-gray-800">
                                <span className="text-gray-300">Category: </span>  {itemArray[0].category}
                            </p>
                            <p className="text-sm leading-none text-gray-800">
                                <span className="text-gray-300">Rating: </span> {itemArray[0].rating}
                            </p>
                        </div>
                    </div>
                    <div className="flex justify-between space-x-8 items-start w-full">
                        <p className="text-base xl:text-lg leading-6">
                        ${(itemArray[0].price * (1 - itemArray[0].discountPercentage / 100) ).toFixed(0)} <span className="text-red-300 line-through"> ${itemArray[0].price}</span>
                        </p>
                        <p className="text-base xl:text-lg leading-6 text-gray-800">{itemArray[0].quantity}</p>
                        <p className="text-base xl:text-lg font-semibold leading-6 text-gray-800">${(itemArray[0].price * (1 - itemArray[0].discountPercentage / 100)* itemArray[0].quantity ).toFixed(0)}</p>
                    </div>
                </div>
            </div>
                        ))}
...
{/* <div><div className="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
        <div className="flex justify-center items-center space-y-2 flex-col ">
            <h1 className="text-xl lg:text-2xl font-semibold leading-7 lg:leading-9   text-gray-800">Order #{digitSum(orderInfo.id)} </h1>
            <p className="text-base font-medium leading-6 text-gray-600">{orderInfo.timestamp}</p>
        </div>
